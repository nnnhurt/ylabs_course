...
def read_menu(menu_id: UUID, db: Session = Depends(get_db)):
    ...
    db_menu.submenu_count_in_menu = crud.submenu_count(db, menu_id=menu_id)
    db_menu.dish_count_in_menu = crud.dish_count(db, menu_id=menu_id)
    return db_menu
...
def read_all_menus(db: Session = Depends(get_db)):
    ...
    for row in menus:
        row.submenu_count_in_menu = crud.submenu_count_in_menu(
            db, menu_id=row.id)
        row.dish_count_in_menu = crud.dish_count_in_menu(db, menu_id=row.id)
    return menus
...
def create_menu(menu: schemas.CreateMenu, db: Session = Depends(get_db)):
    ...
    db_menu.submenu_count_in_menu = (
        crud.submenu_count_in_menu(db, menu_id=db_menu.id)
    )
    db_menu.dish_count_in_menu = crud.dish_count_in_menu(
        db, menu_id=db_menu.id)
    return db_menu
...
def update_menu(menu_id: UUID, menu: schemas.Menu, db: Session = Depends(get_db)):
    ...
    db_menu.submenu_count_in_menu = (
        crud.submenu_count_in_menu(db, menu_id=menu_id))
    db_menu.dish_count_in_menu = crud.dish_count_in_menu(db, menu_id=menu_id)
    return db_menu
