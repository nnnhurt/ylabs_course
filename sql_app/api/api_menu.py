...
def read_menu(menu_id: UUID, db: Session = Depends(get_db)):
    ...
    db_menu.submenus_count = crud.submenu_count_in_menu(
        db, menu_id=menu_id)
    db_menu.dishes_count = crud.dish_count_in_menu(db, menu_id=menu_id)
    return db_menu
...
def read_all_menus(db: Session = Depends(get_db)):
    ...
    for row in menus:
        row.submenus_count = crud.submenu_count_in_menu(
            db, menu_id=row.id)
...
def create_menu(menu: schemas.CreateMenu, db: Session = Depends(get_db)):
    ...
    db_menu.dishes_count = crud.dish_count_in_menu(
